...
TEST(test_initinoutput, init_init_out) {
  ...
  for (const auto &operator_ : operators) {
    LOG(INFO) << "type: " << operator_->type << " name: " << operator_->name;
    const auto &
        output_operands = operator_->output_operands;
    std::string shape_str;
  ...
  // 开始检验输出空间有没有准备好？
  const auto
      &operators_sub = std::vector<std::shared_ptr<RuntimeOperator>>(operators.begin() + 1, operators.begin() + 3);
  ASSERT_EQ(operators_sub.size(), 2);
  const std::shared_ptr<RuntimeOperator> &conv1_operator = *operators_sub.begin();
  const std::shared_ptr<RuntimeOperand> &conv1_output = conv1_operator->output_operands;
  ...
  const std::shared_ptr<RuntimeOperator> &relu_operator = *(operators_sub.begin() + 1);
  const std::shared_ptr<RuntimeOperand> &relu_output = relu_operator->output_operands;
